...
class ScheduleCreateDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        test_layout.addRow("설정 기간:", period_layout)
        ...
        product_layout.addRow("살균여부:", self.sterilization_input)

        # 멸균여부
        self.pasteurization_input = QLineEdit()
        product_layout.addRow("멸균여부:", self.pasteurization_input)
   ...
   # ScheduleCreateDialog 클래스 내부의 search_client 함수 수정
    ...
    def period_value_changed(self, value, period_type):
        ...
        try:
            message = ""
            if period_type == "days":
                message = f"{value}일을 입력하셨습니다."
            elif period_type == "months":
                message = f"{value}개월을 입력하셨습니다."
            elif period_type == "years":
                message = f"{value}년을 입력하셨습니다."
                
            print(f"설정 기간 변경: {message}")  # 로그 기록
            
            # 값이 변경되면 정보 메시지 표시
            if value > 0:
                QMessageBox.information(self, "설정 기간 입력", message)
        except Exception as e:
            print(f"기간 값 변경 처리 중 오류 발생: {str(e)}")  # 로그 기록
